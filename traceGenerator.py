...
def getBounds():
    ...
    #Need to change the file path here#
    path = 'C:/traces/huge_normal'
    dirList = dircache.listdir(path)
    
    lowBoundX = highBoundX = lowBoundY = highBoundY = lowBoundZ = highBoundZ = 0
    
    #for dir in dirList:
      #filelist = dircache.listdir(path+dir+'/')
      
      #for infile in filelist:
    for infile in dirList:
        #bounds for move_left '+' and move_right
        minX = 0
        maxX = 0
        #bounds for move_up '+'and move_down
        minY = 0
        maxY = 0
        #bounds for zoom_in '+'and zoom_out
        minZ = 0
        maxZ = 0
        #initialize to zero
        x = y = z = 0
...
def selectSessionLength():
    ...
    #print 'current session length: ', rand
    return rand 
