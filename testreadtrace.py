import unittest
from readtrace import State, transition
...
class TestReadTrace(unittest.TestCase):
    ...
    def test_transistion_general(self):
        for ops in ["ZOOM_IN", "ZOOM_OUT", "MOVE_UP", "MOVE_DOWN",\
                    ...
                    "RESET", "BEGIN"]:
            state2 = transition(ops, 123456, self.state)
    ...
    def test_count_same(self):
        ...
        state2 = transition("ZOOM_IN", 123456, self.state)
        self.assertEqual(self.state.count + 1, state2.count)

    def test_count_diff(self):
        self.state.last_op = "ZOOM_IN"
        self.state.count = 10
        state2 = transition("ZOOM_OUT", 123456, self.state)
    ...
    def test_move_left(self):
        state2 = transition("MOVE_LEFT", 123456, self.state)
        self.assertEqual(self.state.x - 1, state2.x)
    
    def test_move_right(self):
        state2 = transition("MOVE_RIGHT", 123456, self.state)
        self.assertEqual(self.state.x + 1, state2.x)

    def test_move_up(self):
        state2 = transition("MOVE_UP", 123456, self.state)
        self.assertEqual(self.state.y + 1, state2.y)
    
    def test_move_down(self):
        state2 = transition("MOVE_DOWN", 123456, self.state)
        self.assertEqual(self.state.y - 1, state2.y)

    def test_tilt_forward(self):
        state2 = transition("TILT_FORWARD", 123456, self.state)
        self.assertEqual((self.state.ax - 1)%36, state2.ax)

    def test_tilt_backward(self):
        state2 = transition("TILT_BACKWARD", 123456, self.state)
        self.assertEqual((self.state.ax + 1)%36, state2.ax)

    def test_revolve_clockwise(self):
        state2 = transition("REVOLVE_CLOCKWISE", 123456, self.state)
        self.assertEqual((self.state.ay + 1)%36, state2.ay)
    
    def test_revolve_anticlockwise(self):
        state2 = transition("REVOLVE_ANTICLOCKWISE", 123456, self.state)
        self.assertEqual((self.state.ay - 1)%36, state2.ay)
    
    def test_rotate_clockwise(self):
        state2 = transition("ROTATE_CLOCKWISE", 123456, self.state)
        self.assertEqual((self.state.az + 1)%36, state2.az)
    
    def test_rotate_anticlockwise(self):
        state2 = transition("ROTATE_ANTICLOCKWISE", 123456, self.state)
        self.assertEqual((self.state.az - 1)%36, state2.az)

    def test_reset(self):
        state2 = transition("RESET", 123456, self.state)
        for i in [state2.x, state2.y, state2.z, \
                  state2.ax, state2.ay, state2.az]:
            self.assertEqual(i, 0)
    
    def test_reset(self):
        state2 = transition("BEGIN", 123456, self.state)
        for i in [state2.x, state2.y, state2.z, \
                  state2.ax, state2.ay, state2.az]:
            self.assertEqual(i, 0)
